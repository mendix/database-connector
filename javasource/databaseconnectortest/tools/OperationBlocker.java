...
public class OperationBlocker {
	...
	public static void blockUntilReady(Supplier<Boolean> isBusy, Long sleepTimeInSeconds, ILogNode logNode,
			String name) {
		new OperationBlocker(isBusy, sleepTimeInSeconds, logNode, name).blockUntilReady();
	}

	public void blockUntilReady() {
		Stream.generate(isBusy).peek(this::sleepIfBusy).anyMatch(Predicate.isEqual(false));
